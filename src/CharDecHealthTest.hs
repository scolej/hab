...
decHealthSimple =
  let mods = [ modh (-5) ]
  in TestCase $ assertEqual "Character should decrease in health."
...
decHealthDead1 =
  ...
  in TestCase $ assertEqual "Character should be dead."
             (CharDead 0 1)
             (deriveCharacter mods blankCharacter)
...
decHealthDead3 =
  let mods = [ modh (-fullHealth) ]
  in TestCase $ assertEqual "Character should be dead but level and experience preserved."
             (CharDead 10 5)
             (deriveCharacter mods (CharState 50 10 5))

main :: IO Counts
main = runTestTT $ TestList [ decHealthSimple
                            , decHealthDead1
                            , decHealthDead2
                            , decHealthDead3
                            ]
